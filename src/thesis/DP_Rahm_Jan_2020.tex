...
\abstractEN{The Master's thesis deals with selected algorithms of multivariate cryptography, especially Unbalanced Oil \& Vintage and Rainbow. The aim of this work is implementation of algorithms in Wolfram Mathematica, investigation of existing solutions and their implementation on ESP32 microcontroller. The algorithms are tested and measured against the RSA and ECDSA algorithms.}
...
\bigskip
\noindent
The details of RSA implementation is not subject of this Master's thesis 
...
\bigskip
\noindent
The reference implementation contains several optimized solutions. But for purpose of this Master's thesis is only relevant solution under the folder of \textit{Reference\_Implementation}.
...
To load application to ESP32-LyraT, one must first build the project. For example, copy the project "\$PATH\_TO\_ESP/esp/esp-idf/examples/get-started/hello\_world" to "\$PATH\_TO\_ESP/esp/hello\_world". In this folder it is possible to build it:
\begin{lstlisting}[frame=single]
idf.py -n build
\end{lstlisting}
