...
\end{mmaCell}
From this place on it will only focus on computation of signature $z$ for message $w$. Be aware that in this example is not used hash function for the message because for the example it is not needed.
\begin{mmaCell}[moredefined={w, x1, x2, x3}]{Input}
  w=\{\{3\},\{6\},\{4\}\};
  x1= 1;
  x2= 0;
  x3= 6;
\end{mmaCell}
It sets the message to value $w = (3,6,4)$ and also it set values for $x = (x1,x2,x3)$. These values for $x$ are randomly chosen.
\begin{mmaCell}[addtoindex=3,moredefined={f1, F1, x1, x2, x3, mod, f2, F2, f3, F3}]{Input}
  f1 = PolynomialMod[F1[x1,x2,x3,x4,x5,x6],mod]
  f2 = PolynomialMod[F2[x1,x2,x3,x4,x5,x6],mod]
  f3 = PolynomialMod[F3[x1,x2,x3,x4,x5,x6],mod]
\end{mmaCell}
...
\begin{mmaCell}{Output}
  ...
  f3 = 5+6x4+4x5+x6
\end{mmaCell}
Next two steps solve linear system $f^{(1)} = w_1 = 3, f^{(2)} = w_2 = 6, f^{(3)} = w_3 = 4$, it can also use for solution the Gaussian elimination:
\begin{mmaCell}[moredefined={res, f1, w, f2, f3, mod}]{Input}
  res = Solve[\{f1==w[[1]], f2==w[[2]], f3==w[[3]]\},Modulus \(\pmb{\to}\) mod];
\end{mmaCell}
\begin{mmaCell}[moredefined={x, x1, x2, x3, res}]{Input}
  x=\{x1,x2,x3,x4,x5,x6\} /. res 
\end{mmaCell}
\begin{mmaCell}{Output}
  \{\{1,0,6,6,3,0\}\}
\end{mmaCell}
It will obtain results for $(x4,x5,x6) = (6,3,0)$. After combination it is $x = (1,0,6,6,3,0)$, so called \textit{pre-image} of $w$: $x =  \mathcal{F}^{-1}(w)$. If the solution for linear system do not exist, choose different values for $(x1,x2,x3)$ and repeat steps before.
...
\end{equation*}
	\end{minipage}
\end{figure}
\begin{mmaCell}[moredefined={z, A, x, b, mod}]{Input}
  z = Mod[\mmaSub{A}{-1}.(Transpose[x]-b),mod] 
\end{mmaCell}
\begin{mmaCell}{Output}
  \{\{4\},\{1\},\{5\},\{6\},\{3\},\{5\}\}
\end{mmaCell}
