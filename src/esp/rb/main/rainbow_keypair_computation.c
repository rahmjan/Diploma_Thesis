...
//  Q_pk.l1_F5s[i] = UT( T1tr* (F1 * T1 + F2) )
    const unsigned char * t2 = Ts->t4;
    unsigned char * tempQ_l1_F2 = (unsigned char *) aligned_alloc( 32 , _O1_BYTE * _V1 * _O1 );
    unsigned char * tempQ_l2_F1 = (unsigned char *) aligned_alloc( 32 , _O1_BYTE * _V1 * _O2 );
    memcpy( tempQ_l1_F2 , Fs->l1_F2 , _O1_BYTE * _V1 * _O1 );
    batch_trimat_madd( tempQ_l1_F2 , Fs->l1_F1 , Ts->t1 , _V1, _V1_BYTE , _O1, _O1_BYTE );      // F1*T1 + F2
    memset( tempQ_l2_F1 , 0 , _O1_BYTE * _V1 * _O2 );
    batch_matTr_madd( tempQ_l2_F1 , Ts->t1 , _V1, _V1_BYTE, _O1, tempQ_l1_F2, _O1, _O1_BYTE );  // T1tr*(F1*T1 + F2)
    memset( Qs->l1_Q5 , 0 , _O1_BYTE * N_TRIANGLE_TERMS(_O1) );
    UpperTrianglize( Qs->l1_Q5 , tempQ_l2_F1 , _O1, _O1_BYTE );                        // UT( ... )   // Q5
    free(tempQ_l2_F1);
...
*/
    ...
    memset( tempQ_l1_F2 , 0 , _O1_BYTE * _V1 * _O2 );                                       // should be F3. assuming: _O1 >= _O2
    batch_matTr_madd( tempQ_l1_F2 , t2 , _V1, _V1_BYTE, _O2, Qs->l1_Q3, _O2, _O1_BYTE );    // T2tr * ( F1_T2 + F2_T3 )
    UpperTrianglize( Qs->l1_Q9 , tempQ_l1_F2 , _O2 , _O1_BYTE );                            // Q9
    free(tempQ_l1_F2);
...
/*
    ...
    Q9 = UT( T2tr*( F1*T2 + F2*T3 + F3 )  +  T3tr*( F5*T3 + F6 ) )
*/
    unsigned char * tempQ2_l2_F3 = (unsigned char *) aligned_alloc( 32 , _O2_BYTE * _V1 * _O2 );
    memcpy( tempQ2_l2_F3 , Fs->l2_F3 , _O2_BYTE * _V1 * _O2 );  /// F3 actually.
    batch_trimat_madd( tempQ2_l2_F3 , Fs->l2_F1 , t2 , _V1, _V1_BYTE , _O2, _O2_BYTE );       // F1*T2 + F3
    batch_mat_madd( tempQ2_l2_F3 , Fs->l2_F2 , _V1, Ts->t3 , _O1, _O1_BYTE , _O2, _O2_BYTE ); // F1_T2 + F2_T3 + F3

    unsigned char * tempQ_l2_F3 = (unsigned char *) aligned_alloc( 32 , _O2_BYTE * _V1 * _O2 );
    memset( tempQ_l2_F3 , 0 , _O2_BYTE * _V1 * _O2 );
    batch_matTr_madd( tempQ_l2_F3 , t2 , _V1, _V1_BYTE, _O2, tempQ2_l2_F3, _O2, _O2_BYTE );   // T2tr * ( ..... )
    free(tempQ2_l2_F3);

    unsigned char * tempQ_l2_F6 = (unsigned char *) aligned_alloc( 32 , _O2_BYTE * _O1 *_O2 );
    memcpy( tempQ_l2_F6 , Fs->l2_F6 , _O2_BYTE * _O1 *_O2 );
    batch_trimat_madd( tempQ_l2_F6 , Fs->l2_F5 , Ts->t3 , _O1, _O1_BYTE, _O2, _O2_BYTE );     // F5*T3 + F6

    batch_matTr_madd( tempQ_l2_F3 , Ts->t3 , _O1, _O1_BYTE, _O2, tempQ_l2_F6, _O2, _O2_BYTE ); // T2tr*( ..... ) + T3tr*( ..... )
    memset( Qs->l2_Q9 , 0 , _O2_BYTE * N_TRIANGLE_TERMS(_O2) );
    UpperTrianglize( Qs->l2_Q9 , tempQ_l2_F3 , _O2 , _O2_BYTE );                              // Q9
